...
public ref struct KafkaProtocolReader
{
    ...
    public Guid ReadUuid()
    {
        if (_isContiguous)
        {
            if (_position + 16 > _span.Length)
                ThrowInsufficientData();
            var result = Dekaf.KafkaGuid.ReadBigEndian(_span.Slice(_position, 16));
            _position += 16;
            return result;
        }
        return ReadUuidSlow();
    ...
    private Guid ReadUuidSlow()
    {
        ...
        {
            var result = Dekaf.KafkaGuid.ReadBigEndian(_reader.UnreadSpan[..16]);
            _reader.Advance(16);
            return result;
        }
        ...
        if (!_reader.TryCopyTo(buffer))
            ThrowInsufficientData();
        _reader.Advance(16);
        return Dekaf.KafkaGuid.ReadBigEndian(buffer);
    }
